...
'''Lists can be created in several ways, such as using square brackets, 
   the list() constructor or by repeating elements. Let's look at each method one by one with example:
   1. Using Square Brackets
   2. Using list() Constructor
   3. Creating List with Repeated Elements
   '''
#Accessing List Elements
'''Elements in a list are accessed using indexing. Python indexes start at 0, so a[0] gives the first element. 
   Negative indexes allow access from the end (e.g., -1 gives the last element).'''
